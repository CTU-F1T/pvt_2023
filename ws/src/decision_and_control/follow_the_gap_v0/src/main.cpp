...
std::vector<FollowTheGap::Obstacle> CreateObstacles(obstacle_msgs::ObstaclesStamped::ConstPtr const & obstacles_data) {
    size_t const data_size = obstacles_data->obstacles.circles.size();

    std::vector<FollowTheGap::Obstacle> obstacles;
    for ( size_t i = 0; i < data_size; ++i ) {
        obstacles.emplace_back(obstacles_data->obstacles.circles[i].center.x, obstacles_data->obstacles.circles[i].center.y, obstacles_data->obstacles.circles[i].radius, FollowTheGap::kCarRadius);
    }
    // We reverse the array so that the first obstacles are on the left(largest angle, in accordance with the paper)
...
void PublishVisualizeFinalHeadingAngle(float final_heading_angle, std::string frame_id) {
...
void PublishVisualizeLargestGap(FollowTheGap::Obstacle const & gap_left, FollowTheGap::Obstacle const & gap_right, std::string frame_id) {
...
void PublishVisualizeObstacles(std::vector<FollowTheGap::Obstacle> const & obstacles, std::string frame_id) {
...
void PublishGapFound(bool gap_found) {
    std_msgs::Bool gap_found_message;
...
void ObstaclesCallback(obstacle_msgs::ObstaclesStamped::ConstPtr const & obstacles_data) {
    ...
    std::tie(ok, angle) = FollowTheGap::Callback(obstacles, NULL, obstacles_out, gap_borders_out);
...
/*
    ...
    //     ->advertise<std_msgs::Bool>("/gap_found", kPublishMessageBufferSize);
