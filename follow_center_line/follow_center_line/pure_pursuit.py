...
class PurePursuit(Node):
    ...
    def __init__(self):
        ...
        self.drive_publisher = self.create_publisher(DriveApiValues, drive_topic, 1)
        self.way_pub = self.create_publisher(Marker, 'lookahead', 1)
        self.path_publisher = self.create_publisher(PointCloud2, '/path_tf', 1)
        ...
        qos_profile = QoSProfile(depth=1)
        self.subscription = self.create_subscription(PointCloud2, '/path', self.path_callback, 1)
    ...
    def control(self):
        ...
        if angle < 0:
            ...
            dv_msg.steering = float(angle)
            dv_msg.right = False
        dv_msg.steering = np.clip(k*dv_msg.steering, -1, 1)
        # dv_msg.velocity = np.exp(-k*np.abs(angle))
        # dv_msg.velocity = np.clip(dv_msg.velocity, 0, 0.4)
        dv_msg.velocity = 0.3
       	if abs(dv_msg.steering) > 0.2:
            dv_msg.velocity = 0.15
        #if abs(dv_msg.steering) > 0.3:
        #    dv_msg.velocity = 0.2
        #if val[i_min] > la:
        #    dv_msg.velocity = 0.0
	#    self.get_loger().info("Vzdalenost od nejblizsiho ",val[i_min])
        dv_msg.forward = True
        self.drive_publisher.publish(dv_msg)
        self.get_logger().info('published msg %s' % (str(dv_msg.velocity)))
